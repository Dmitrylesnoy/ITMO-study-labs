...
class InterpolationMath:
    ...
    def get_finite_diffs(y):
        ...
        for j in range(1, n):
            prev = table[j - 1]
            current = np.array([prev[i + 1] - prev[i] for i in range(len(prev) - 1)])
    ...
    def newton_divided(x_pts, y_pts, x_val):
        ...
        if x_val <= x_pts[mid]:
            ...
            for i in range(1, n):
                ...
                res += coef[n-1-i, i] * product
            return res, "Ньютон (р.р. назад)"
...
class InterpolationApp(tk.Tk):
    ...
    def show_tables_window(self, x, y):
        ...
        self._build_treeview(
            f_diff_frame, x, InterpolationMath.get_finite_diffs(y), is_finite=True
        )
        ...
        self._build_treeview(
            d_diff_frame, x, InterpolationMath.get_divided_diffs(x, y), is_finite=False
        )
        ...
        try:
            if self.input_mode.get() == "manual":
                ...
                y = (
                    sin(x)
                    if "sin" in f_name
                    else (exp(x) if "exp" in f_name else cos(x))
                )

            tx = float(self.target_x.get())
            diffs = InterpolationMath.get_finite_diffs(y)

            v_newt_fin, nf_name = InterpolationMath.newton_finite_auto(x, y, tx, diffs)
            v_newt_div, nd_name = InterpolationMath.newton_divided(x, y, tx)
